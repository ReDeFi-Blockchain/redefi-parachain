...
use crate::{
	...
	service::{new_partial, start_dev_node, start_node},
};
...
fn load_spec(id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
	Ok(match id {
		...
		path => {
			...
			Box::new(chain_spec::ChainSpec::from_json_file(path.clone())?)
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			match cmd {
				...
				BenchmarkCmd::Block(cmd) => runner.sync_run(|config| {
					...
					>(
						&config,
						crate::service::parachain_build_import_queue::<redefi_runtime::Runtime, _, _>,
					)?;
